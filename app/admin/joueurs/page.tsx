...
export default async function JoueursPage() {
  ...
  const { data: memberships } = await supabase
    .from("team_members")
    .select("*, teams(id, name, colors_primary, entreprise_id)")

  return (
    <div className="flex-1 p-6 lg:p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-foreground">Joueurs</h1>
        <p className="mt-1 text-muted-foreground">Creez et gerez les comptes joueurs, leurs entreprises, equipes et postes</p>
      </div>
      <PlayersManager
