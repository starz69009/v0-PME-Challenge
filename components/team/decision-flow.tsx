...
import { CheckCircle, ShieldCheck, Clock, User, ThumbsUp, ThumbsDown, Timer, AlertTriangle, MessageSquare, ChevronDown, ChevronUp, Loader2, RefreshCw } from "lucide-react"
...
function CountdownTimer({ expiresAt, onExpire, durationSeconds }: { expiresAt: string; onExpire: () => void; durationSeconds: number | null }) {
  ...
  const isCritical = timeLeft <= Math.max(10, Math.floor((durationSeconds || 300) * 0.03))
  ...
  return (
    <div className={`flex items-center gap-3 rounded-xl border px-5 py-3 font-mono transition-all ${
      isCritical ? "border-destructive/50 bg-destructive/10 animate-pulse" : isUrgent ? "border-warning/50 bg-warning/10" : "border-primary/30 bg-primary/5"
    }`}>
...
async function apiGet(sessionEventId: string, teamId: string) {
  const res = await fetch(`/api/decisions?sessionEventId=${sessionEventId}&teamId=${teamId}`)
  if (!res.ok) return null
  return res.json()
}

// ---------- Main Component ----------
export function DecisionFlow({ sessionEvent, currentUserId, currentRole, teamId, teamMembers, expiresAt, durationSeconds }: Props) {
  // Data state -- fetched entirely from API
  ...
  const [specAvantages, setSpecAvantages] = useState("")
  const [specInconvenients, setSpecInconvenients] = useState("")
  const [specJustification, setSpecJustification] = useState("")
  ...
  const [expired, setExpired] = useState(() => expiresAt ? new Date(expiresAt).getTime() <= Date.now() : false)
  ...
  const refreshData = useCallback(async () => {
    ...
    try {
      const result = await apiGet(sessionEvent.id, teamId)
      if (!result || !mountedRef.current) return
      setDecision(result.decision)
      setVotes(result.votes || [])
      setFetchError(null)
    } catch (err: any) {
      if (mountedRef.current) setFetchError(err.message)
    }
  }, [sessionEvent.id, teamId])
  ...
  async function proposeOption(optionId: string) {
    ...
    if (!decision?.id) { toast.error("Decision non disponible, veuillez patienter"); return }
    if (!specAvantages.trim() || !specInconvenients.trim() || !specJustification.trim()) {
      toast.error("Veuillez remplir les 3 champs d'argumentaire")
      return
    }
    setLoading(true)
    try {
      const result = await apiPost({
        action: "propose", decisionId: decision.id, optionId,
        avantages: specAvantages, inconvenients: specInconvenients, justification: specJustification,
      })
  ...
  async function castVote() {
    if (isLocked) { toast.error("Le temps est ecoule"); return }
    if (!decision?.id) { toast.error("Decision non disponible"); return }
    if (voteApproved === null) { toast.error("Veuillez approuver ou rejeter"); return }
    if (!voteComment.trim()) { toast.error("Veuillez rediger un commentaire"); return }
    setLoading(true)
    try {
      await apiPost({ action: "vote", decisionId: decision.id, optionId: decision.proposed_option_id, approved: voteApproved, comment: voteComment })
  ...
  async function dgValidate() {
    ...
    try {
      ...
      const result = await apiPost({ action: "validate", decisionId: decision.id, dgComment, overrideOptionId: overrideId })
      if (result.decision) setDecision(result.decision)
    ...
    <div className="space-y-6">
        ...
        <Card className="border-destructive/30 bg-destructive/5">
          <CardContent className="flex items-center gap-4 p-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{"Le delai est termine."}</p>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      <div className="flex items-center justify-center gap-2">
        <Badge variant="outline" className="border-primary/30 text-primary text-xs px-3 py-1">
          Votre role : {COMPANY_ROLE_LABELS[currentRole]}
          {isSpecialist && " (Specialiste)"}
          {isDG && " (DG)"}
        </Badge>
      </div>
        ...
        <Card className="border-primary/20 bg-primary/5">
          <CardContent className="flex flex-col items-center gap-4 p-8 text-center">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">Recuperation des donnees de la decision.</p>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      {!dataLoading && fetchError && !decision && (
        <Card className="border-destructive/20 bg-destructive/5">
          <CardContent className="flex flex-col items-center gap-4 p-8 text-center">
            ...
            <Button variant="outline" size="sm" onClick={() => { setDataLoading(true); refreshData().finally(() => setDataLoading(false)) }}>
      ...
      {/* Decision not created yet */}
      {!dataLoading && !fetchError && !decision && (
        <Card className="border-primary/20 bg-primary/5">
          <CardContent className="flex flex-col items-center gap-4 p-8 text-center">
            <Loader2 className="h-8 w-8 text-primary animate-spin" />
            <div>
              <p className="text-sm font-bold text-foreground">En attente...</p>
              <p className="mt-1 text-xs text-muted-foreground">{"La decision de votre equipe n'a pas encore ete creee. Rafraichissement automatique..."}</p>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      {/* ===== DECISION EXISTS ===== */}
      {!dataLoading && decision && (
        <>
          {/* Progress Steps */}
          <div className="flex items-center justify-center gap-0">
            {steps.map((step, idx) => (
              <div key={step.label} className="flex items-center">
                {idx > 0 && <div className={`h-0.5 w-8 sm:w-12 transition-colors ${step.done || step.active ? "bg-primary" : "bg-border/60"}`} />}
                <div className={`flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[10px] sm:text-xs font-semibold transition-all ${
                  step.done ? "bg-success/15 text-success" : step.active ? "bg-primary/15 text-primary" : "bg-muted/50 text-muted-foreground"
                }`}>
                ...
                <Card className="border-primary/30 bg-primary/5">
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription className="text-xs">
                      En tant que <span className="font-semibold text-foreground">{COMPANY_ROLE_LABELS[specialistRole]}</span>, selectionnez une option et argumentez. <span className="font-semibold text-foreground">Votre choix sera definitif.</span>
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div className="grid gap-3 sm:grid-cols-3">
                      {options.map((option: any) => (
                        <div key={option.id} onClick={() => !isLocked && !loading && setSelectedOption(option.id)}
                          className={`cursor-pointer rounded-lg border p-3 transition-all ${
                            selectedOption === option.id ? "border-primary/50 bg-primary/10 ring-2 ring-primary/20" : "border-border/40 bg-card/60 hover:border-primary/25"
                          }`}>
                    ...
                    {selectedOption && (
                      <div className="space-y-3 rounded-lg border border-border/30 bg-muted/10 p-4">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <Textarea placeholder="Quels sont les points positifs ?" value={specAvantages} onChange={(e) => setSpecAvantages(e.target.value)} rows={2} className="bg-secondary/50 border-border/40 text-sm" />
                        </div>
                        <div className="space-y-1.5">
                          <Label className="text-xs font-semibold text-destructive">Inconvenients de ce choix</Label>
                          <Textarea placeholder="Quels risques ou points negatifs ?" value={specInconvenients} onChange={(e) => setSpecInconvenients(e.target.value)} rows={2} className="bg-secondary/50 border-border/40 text-sm" />
                        </div>
                        <div className="space-y-1.5">
                          <Label className="text-xs font-semibold text-primary">{"Qu'est-ce qui justifie cette decision ?"}</Label>
                          <Textarea placeholder="Pourquoi cette option est la meilleure..." value={specJustification} onChange={(e) => setSpecJustification(e.target.value)} rows={2} className="bg-secondary/50 border-border/40 text-sm" />
                        </div>
                        <Button className="w-full glow-primary" onClick={() => proposeOption(selectedOption)} disabled={loading || isLocked}>
            ...
            <>
                ...
                <CardContent className="space-y-3">
                  ...
                  {(decision?.comment_avantages || decision?.comment_inconvenients || decision?.comment_justification) && (
                    <div className="space-y-2 rounded-lg border border-border/30 bg-muted/10 p-3">
                      <p className="text-[10px] uppercase tracking-wider font-bold text-muted-foreground">Argumentaire</p>
                      {decision.comment_avantages && <div className="space-y-0.5"><p className="text-[10px] font-semibold text-success">Avantages</p><p className="text-xs text-muted-foreground leading-relaxed">{decision.comment_avantages}</p></div>}
                      {decision.comment_inconvenients && <div className="space-y-0.5"><p className="text-[10px] font-semibold text-destructive">Inconvenients</p><p className="text-xs text-muted-foreground leading-relaxed">{decision.comment_inconvenients}</p></div>}
                      {decision.comment_justification && <div className="space-y-0.5"><p className="text-[10px] font-semibold text-primary">Justification</p><p className="text-xs text-muted-foreground leading-relaxed">{decision.comment_justification}</p></div>}
                    </div>
                  )}
                </CardContent>
              </Card>
              ...
              <Card className="border-border/40 bg-card/80">
                ...
                <CardContent className="space-y-3">
                  ...
                  {votes.length > 0 && (
                    <button onClick={() => setShowVoteDetails(!showVoteDetails)} className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors">
                    ...
                    return (
                      <div key={v.id} className="rounded-lg border border-border/20 bg-muted/10 p-3 space-y-1.5">
                        <div className="flex items-center justify-between">
                          ...
                          {v.approved ? <Badge className="bg-success/15 text-success text-[10px] border-success/30"><ThumbsUp className="h-2.5 w-2.5 mr-1" />Pour</Badge>
                        ...
                        {v.comment && <div className="flex items-start gap-1.5"><MessageSquare className="h-3 w-3 text-muted-foreground/50 mt-0.5 shrink-0" /><p className="text-xs text-muted-foreground italic">{v.comment}</p></div>}
                      </div>
                    )
                  })}
                </CardContent>
              ...
              {/* Voter form -- for non-DG, non-specialist, who haven't voted */}
              {!isDG && !isSpecialist && !userVote && !isLocked && (
                <Card className="border-primary/30 bg-primary/5">
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription className="text-xs">Approuvez-vous cette proposition ? Argumentez du point de vue de votre role.</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div className="flex gap-3">
                      <Button variant={voteApproved === true ? "default" : "outline"} className={`flex-1 ${voteApproved === true ? "bg-success hover:bg-success/90 text-success-foreground" : ""}`} onClick={() => setVoteApproved(true)}>
                        <ThumbsUp className="mr-2 h-4 w-4" /> Approuver
                      </Button>
                      <Button variant={voteApproved === false ? "default" : "outline"} className={`flex-1 ${voteApproved === false ? "bg-destructive hover:bg-destructive/90 text-destructive-foreground" : ""}`} onClick={() => setVoteApproved(false)}>
                        <ThumbsDown className="mr-2 h-4 w-4" /> {"S'opposer"}
                      </Button>
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs font-semibold text-muted-foreground">Commentaire (du point de vue : {COMPANY_ROLE_LABELS[currentRole]})</Label>
                      <Textarea placeholder={`En tant que ${COMPANY_ROLE_LABELS[currentRole]}, je pense que...`} value={voteComment} onChange={(e) => setVoteComment(e.target.value)} rows={3} className="bg-secondary/50 border-border/40 text-sm" />
                    </div>
              ...
              {!isDG && userVote && (
                <Card className="border-success/30 bg-success/5">
                  <CardContent className="flex items-center gap-4 p-4">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-success/15"><CheckCircle className="h-5 w-5 text-success" /></div>
                    <div>
                      <p className="text-sm font-bold text-foreground">Vote enregistre</p>
                      <p className="text-xs text-muted-foreground">Vous avez {userVote.approved ? "approuve" : "rejete"} la proposition. En attente de la decision du DG.</p>
                    </div>
                  </CardContent>
                </Card>
              )}
              ...
              {isSpecialist && !isDG && (
                <Card className="border-border/30 bg-card/40">
                  <CardContent className="flex items-center gap-4 p-4">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-muted/50"><Clock className="h-5 w-5 text-muted-foreground/50" /></div>
                    <div>
              ...
              {isDG && !isLocked && (
                <Card className="border-warning/30 bg-warning/5">
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription className="text-xs">
                      {allVotersVoted ? "Tous les membres ont vote. Prenez votre decision." : `En attente (${votes.length}/${nonDGNonSpecMembers.length}). Vous pouvez decider a tout moment.`}
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div className="flex gap-3">
                      <Button variant={!dgOverride ? "default" : "outline"} className={`flex-1 text-xs ${!dgOverride ? "bg-success hover:bg-success/90 text-success-foreground" : ""}`}
                        onClick={() => { setDgOverride(false); setDgOverrideOption(null) }}>
                        <ThumbsUp className="mr-1.5 h-3.5 w-3.5" /> Valider la proposition
                      </Button>
                      <Button variant={dgOverride ? "default" : "outline"} className={`flex-1 text-xs ${dgOverride ? "bg-warning hover:bg-warning/90 text-warning-foreground" : ""}`}
                        onClick={() => setDgOverride(true)}>
                        <ShieldCheck className="mr-1.5 h-3.5 w-3.5" /> Decision unilaterale
                      </Button>
                    </div>
                    {dgOverride && (
                      <div className="space-y-2 rounded-lg border border-warning/30 bg-warning/5 p-3">
                        ...
                        <div className="grid gap-2 sm:grid-cols-3">
                          {options.map((option: any) => (
                            <div key={option.id} onClick={() => setDgOverrideOption(option.id)}
                              className={`cursor-pointer rounded-lg border p-2.5 transition-all text-xs ${dgOverrideOption === option.id ? "border-warning/50 bg-warning/10 ring-1 ring-warning/30" : "border-border/40 bg-card/60 hover:border-warning/25"}`}>
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-xs font-semibold text-muted-foreground">{dgOverride ? "Justification de la decision unilaterale" : "Commentaire de validation"}</Label>
                      <Textarea placeholder={dgOverride ? "Pourquoi vous prenez une decision differente..." : "Commentaire sur cette decision collegiale..."} value={dgComment} onChange={(e) => setDgComment(e.target.value)} rows={3} className="bg-secondary/50 border-border/40 text-sm" />
                    </div>
                    <Button onClick={dgValidate} disabled={loading || (dgOverride && !dgOverrideOption)} className="w-full glow-primary">
