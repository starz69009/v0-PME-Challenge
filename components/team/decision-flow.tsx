...
import { CheckCircle, ShieldCheck, Clock, User, ThumbsUp, ThumbsDown, Timer, AlertTriangle, MessageSquare, ChevronDown, ChevronUp, Loader2 } from "lucide-react"
...
async function apiFetch(sessionEventId: string, teamId: string) {
...
export function DecisionFlow({
  ...
  // Polling via API route (no SSR dependency)
  const refreshData = useCallback(async () => {
    if (!mountedRef.current) return
    const result = await apiFetch(sessionEvent.id, teamId)
  ...
  // STEP 1: Specialist proposes via API
  async function proposeOption(optionId: string) {
    if (isLocked) { toast.error("Le temps est ecoule"); return }
    if (!decision?.id) { toast.error("Decision non trouvee. Veuillez patienter."); return }
    ...
    try {
      const result = await apiCall({
        ...
        optionId,
        avantages: specAvantages,
        inconvenients: specInconvenients,
        justification: specJustification,
      })
      if (result.decision) setDecision(result.decision)
  ...
  async function castVote() {
    if (isLocked) { toast.error("Le temps est ecoule"); return }
    if (!decision?.id) { toast.error("Decision non trouvee"); return }
    ...
    try {
      await apiCall({
        action: "vote",
        decisionId: decision.id,
        optionId: decision.proposed_option_id,
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Decision not yet created by admin */}
      {!decision && (
        <Card className="border-primary/20 bg-primary/5">
          <CardContent className="flex flex-col items-center gap-4 p-8 text-center">
            <Loader2 className="h-8 w-8 text-primary animate-spin" />
            <div>
              <p className="text-sm font-bold text-foreground">En attente de la decision...</p>
              <p className="mt-1 text-xs text-muted-foreground">{"La decision de votre equipe est en cours de creation par l'administrateur."}</p>
            </div>
          </CardContent>
        </Card>
      )}
                ...
                <Card className="border-primary/30 bg-primary/5">
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription className="text-xs">
                      En tant que <span className="font-semibold text-foreground">{COMPANY_ROLE_LABELS[specialistRole]}</span>, vous etes le specialiste. Selectionnez une option et argumentez. <span className="font-semibold text-foreground">Votre choix sera definitif.</span>
                    </CardDescription>
                  </CardHeader>
                    ...
                    {selectedOption && (
                      <div className="space-y-3 rounded-lg border border-border/30 bg-muted/10 p-4">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <Textarea
                            placeholder="Expliquez pourquoi cette option est la meilleure..."
              ...
              {!isDG && !isSpecialist && !userVote && !isLocked && (
                <Card className="border-primary/30 bg-primary/5">
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription className="text-xs">
                      Approuvez-vous cette proposition ? Redigez un commentaire en lien avec votre role.
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Textarea
                        placeholder={dgOverride
                          ? "Expliquez pourquoi vous prenez une decision differente..."
